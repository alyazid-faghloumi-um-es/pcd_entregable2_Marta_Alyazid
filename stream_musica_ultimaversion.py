...
# presentacion de las sesiones 

def mostrar_perfil(nombre_usuario, sesion):
    ...
    print("  ATRIBUTO SONORO       - media - desviacion")
    print("  " + "-" * 43)
    for k in campos_s:
        print(f"  {k:<22} {round(s['media'][k], 2):<7} {round(s['desviacion'][k], 2)}")

    print("\n  ATRIBUTO SENTIMENTAL  - media - desviacion")
    print("  " + "-" * 43)
    for k in campos_e:
        print(f"  {k:<22} {round(e['media'][k], 2):<7} {round(e['desviacion'][k], 2)}")
...
# corutinas asincronas ,una por cada perfil de usuario
#
# Cada corutina simula la sesión de escucha de un usuario:
#   1. Escucha sus canciones una a una con await asyncio.sleep() entre ellas,
#      lo que cede el control al event-loop para que los demás usuarios
#      puedan avanzar al mismo tiempo (concurrencia real entre corutinas).
#   2. Calcula el perfil de la sesión.
#   3. Pide recomendaciones con distintas estrategias.
#
# asyncio.gather() lanza las tres corutinas de forma concurrente dentro
# del mismo hilo, sin necesitar threads ni procesos adicionales.


async def sesion_adam(cat, sistema, manejador_sonoro):
    """Perfil METAL / HARD-ROCK — escucha 4 canciones con pausa entre ellas."""
    print("\n" + "═" * 62)
    print(" ASYNC P1 — perfil METAL / HARD-ROCK  [Adam]")
    print("═" * 62)

    sesion = SesionEscucha(ventana_max=4, manejador=manejador_sonoro)
    adam   = Usuario(id=1, nombre="Adam",
                     sesion=sesion, fecha_registro=datetime(2024, 1, 1))

    for c in [c01, c03, c05, c07]:
        adam.escuchar_cancion(c, datetime.now())
        print(f"  [Adam]  ♪  {c.titulo}")
        await asyncio.sleep(0.1)   # ← cede el control; otras corutinas avanzan

    mostrar_perfil("Adam", sesion)

    sistema.establecer_estrategia(orden_alfabetico())
    print("\n  ── Alfabética ──────────────────────────────────────")
    mostrar_recomendaciones(sistema, sesion)
...
async def sesion_ana(cat, sistema, manejador_sonoro):
    """Perfil PSICODÉLICO / NOSTÁLGICO — escucha 4 canciones con pausa entre ellas."""
    print("\n" + "═" * 62)
    print(" ASYNC P2 — perfil PSICODÉLICO / NOSTÁLGICO  [Ana]")
    print("═" * 62)

    sesion = SesionEscucha(ventana_max=4, manejador=manejador_sonoro)
    ana    = Usuario(id=2, nombre="Ana",
                     sesion=sesion, fecha_registro=datetime(2024, 2, 1))

    for c in [c14, c16, c08, c09]:
        ana.escuchar_cancion(c, datetime.now())
        print(f"  [Ana]   ♪  {c.titulo}")
        await asyncio.sleep(0.1)   # ← cede el control

    mostrar_perfil("Ana", sesion)
...
async def sesion_luis(cat, sistema, manejador_sonoro):
    """Perfil POP / FUNK / BAILABLE — escucha 4 canciones con pausa entre ellas."""
    print("\n" + "═" * 62)
    print(" ASYNC P3 — perfil POP / FUNK / BAILABLE  [Luis]")
    print("═" * 62)

    sesion = SesionEscucha(ventana_max=4, manejador=manejador_sonoro)
    luis   = Usuario(id=3, nombre="Luis",
                     sesion=sesion, fecha_registro=datetime(2024, 3, 1))

    for c in [c11, c12, c18, c20]:
        luis.escuchar_cancion(c, datetime.now())
        print(f"  [Luis]  ♪  {c.titulo}")
        await asyncio.sleep(0.1)   # ← cede el control

    mostrar_perfil("Luis", sesion)
    ...
    ru_c = luis.pedir_recomendacion(RecomendarCancion (cat, sesion, forma_aleatoria()))
    ru_a = luis.pedir_recomendacion(RecomendadorArtista(cat, sesion, forma_aleatoria()))
    ru_p = luis.pedir_recomendacion(RecomendadorPlaylist(cat, sesion, forma_aleatoria()))
...
async def main_async(cat, sistema, manejador_sonoro):
    """
    Punto de entrada asíncrono.

    asyncio.gather() ejecuta las tres sesiones de usuario de forma
    CONCURRENTE: cuando una corutina hace `await asyncio.sleep(...)`,
    el event-loop pasa el control a otra corutina pendiente, de modo
    que los tres usuarios avanzan entrelazados sin bloquearse entre sí.
    """
    print("\n" + "=" * 62)
    print("  inicio de sesiones asincronas concurrentes ")
    print("  (Las tres sesiones de usuario corren al mismo tiempo)")
    print("=" * 62)

    await asyncio.gather(
        sesion_adam(cat, sistema, manejador_sonoro),
        sesion_ana (cat, sistema, manejador_sonoro),
        sesion_luis(cat, sistema, manejador_sonoro),
    )

    print("\n" + "=" * 62)
    print("  todas las sesiones han finalizado")
    print("=" * 62)
...
if __name__ == "__main__":
    ...
    #
    # asyncio.run() crea el event-loop, lanza main_async y lo cierra al acabar.
    # Dentro, gather() ejecuta las tres sesiones de usuario concurrentemente:
    # cada vez que una corutina hace `await asyncio.sleep(...)`, el event-loop
    # da paso a otra, entrelazando su ejecución sin hilos ni procesos extra.
    # ─────────────────────────────────────────────────────────────────────────
    asyncio.run(main_async(cat, sistema, manejador_sonoro))
    ...
    try:
        ...
        def lanzar_productor():
            ...
            prod.producer.flush()
            prod.producer.close()
            print("[PRODUCER] Publicación completada.")

        print("\n" + "=" * 62)
        print("  kafka en tiempo real ")
        print("=" * 62)
        ...
        print(f"\n[KAFKA] Módulo no encontrado: {e}")
        print(" kafka-python instalado:  pip install kafka-python\n")